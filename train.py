...
BASE_FEATURES = [
    ...
    "same_hand", "batter_avg", "pitcher_pitch_count", "pitcher_tired",
...
def prepare_xy(df: pd.DataFrame):
    # Only encode CAT_FEATURES columns that exist in the dataframe
    ...
    # Only keep BASE_FEATURES that exist
...
def add_pitcher_tendencies(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Calculate each pitcher's overall pitch type rates
    ...
    # Rolling mean per pitcher (use expanding to avoid leakage)
    df = df.sort_values(["game_date", "game_pk", "pitcher",
                         "at_bat_number", "pitch_number"])
    ...
    for col in tend_cols:
        df[f"pitcher_{col}"] = (
            ...
            .fillna(0.125)   # default to uniform if no history
        )

    # Drop the raw dummies
...
def train(X_train, y_train, num_classes, le):
    ...
    # Custom weights — boost rare pitches gently, don't kill Fastball
    class_counts = np.bincount(y_tr)
    total = len(y_tr)
    # Mild inverse frequency — square root dampens the effect
    raw_weights = total / (num_classes * class_counts)
    mild_weights = np.sqrt(raw_weights)
    mild_weights = mild_weights / mild_weights.mean()  # normalize around 1.0
